  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "VGA detect: DeviceId: %x VendorId: %x SubClass: %x", DeviceId, VendorId, SubClass));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "QemuVideo: read pci io\n"));
  if (EFI_ERROR (Status)) {
    goto Done;
  }
  DEBUG ((DEBUG_INFO, "QemuVideo: class1: %d class 2: %d Vendor: %lx Devicd: %lx\n", Pci.Hdr.ClassCode[2], Pci.Hdr.ClassCode[1], Pci.Hdr.VendorId, Pci.Hdr.DeviceId));

  Status = EFI_UNSUPPORTED;
  if (!IS_PCI_DISPLAY (&Pci)) {
    DEBUG ((DEBUG_INFO, "QemuVideo: wrong subclass\n"));
    goto Done;
  }
