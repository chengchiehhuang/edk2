...
**/
EFI_STATUS
MemInfoCallback(IN MEMROY_MAP_ENTRY *MemoryMapEntry, IN VOID *Params) {
  EFI_PHYSICAL_ADDRESS Base;
  EFI_RESOURCE_TYPE Type;
  UINT64 Size;
  EFI_RESOURCE_ATTRIBUTE_TYPE Attribue;

  Type = (MemoryMapEntry->Type == 1) ? EFI_RESOURCE_SYSTEM_MEMORY
...
**/
RETURN_STATUS
ParseAcpiInfo(IN UINT64 AcpiTableBase, OUT ACPI_BOARD_INFO *AcpiBoardInfo) {
...
Done:
  ...
  if (MmCfgHdr != NULL) {
    ...
    AcpiBoardInfo->PcieBaseSize =
        (MmCfgBase->EndBusNumber + 1 - MmCfgBase->StartBusNumber) * 4096 * 32 *
        8;
  } else {
  ...
  if (!(Fadt->Flags & EFI_ACPI_5_0_HW_REDUCED_ACPI) && (Fadt->SmiCmd == 0) &&
      !SciEnabled) {
    //
...
BuildHobFromBl(VOID) {
  ...
  DEBUG((DEBUG_INFO, "ParseMmoryInfo done\n"));
  //
  ...
  if (!EFI_ERROR(Status)) {
    NewSysTableInfo =
        BuildGuidHob(&gUefiSystemTableInfoGuid, sizeof(SYSTEM_TABLE_INFO));
    ...
    DEBUG((DEBUG_INFO, "No system tables found..\n"));
  }
  ...
  if (!EFI_ERROR(Status)) {
    NewAcpiBoardInfo =
        BuildGuidHob(&gUefiAcpiBoardInfoGuid, sizeof(ACPI_BOARD_INFO));
...
/**
  This function will build some generic HOBs that doesn't depend on information
from bootloaders.
...
VOID BuildGenericHob(VOID) {
  ...
  BuildMemoryAllocationHob(PcdGet32(PcdPayloadFdMemBase),
                           PcdGet32(PcdPayloadFdMemSize), EfiBootServicesData);
...
EFI_STATUS
EFIAPI
PayloadEntry(IN UINTN BootloaderParameter) {
  EFI_STATUS Status;
  ...
  DEBUG((DEBUG_INFO, "finish InitializeFloatingPointUnits\n"));
  ...
  // DXE core assumes the memory below HOB region could be used, so include the
  // FV region memory into HOB range.
  MemSize = HobMemTop - MemBase;

  DEBUG((DEBUG_INFO, "HobConstructor\n"));
  HandoffHobTable = HobConstructor((VOID *)MemBase, MemSize, (VOID *)HobMemBase,
                                   (VOID *)HobMemTop);

  DEBUG((DEBUG_INFO, "BuildHobFromBl\n"));
  ...
  DEBUG((DEBUG_INFO, "BuildGenericHob\n"));
  ...
  DEBUG((DEBUG_INFO, "LoadDxeCore: 0x%lx\n", DxeCoreEntryPoint));
  // Load the DXE Core
